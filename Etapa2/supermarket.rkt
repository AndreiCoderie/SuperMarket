    ...
    (lambda (n-items)
      (lambda (C)
        (if (null? (counter-queue C))
            (if (zero? (counter-et C)) (make-counter (counter-index C) (counter-tt ((tt+ n-items) C)) n-items (list (cons name n-items) ))
...
; TODO
; Întrucât vom folosi atât min-tt (implementat în etapa 1) cât și  (funcție nouă),
...
(define (general-func-helper f L checker acc)
  ...
  (define (nonNullQueue-helper L acc)
    (cond
      ((null? L) acc)
      (else
       (if (null? (counter-queue (car L))) (nonNullQueue-helper (cdr L) acc) 
...
(define (remove-first-from-counter C)
    ...
    [(counter index tt et queue)
     (cond
       ((null? (counter-queue C)) (struct-copy counter C (index (counter-index C)) (tt 0) (et 0) (queue '())))
       (else
         
     (if (null? (cdr (counter-queue C)))
         (struct-copy counter C (index (counter-index C)) (tt 0) (et 0) (queue '()))
  (struct-copy counter C (index (counter-index C)) (tt (apply + (map cdr (cdr (counter-queue C))))) (et (cdr (car (cdr (counter-queue C))))) (queue (cdr (counter-queue C)))))))]))
  ...
  (if (null? requests)
      ...
      [(list 'ensure average)
        (if (> (+ (averageee (append fast-counters slow-counters))) average)
            (serve requests fast-counters (append slow-counters (list (empty-counter (+ (counter-index (last_element slow-counters)) 1)))))
       ...
       [(list 'delay index minutes)
        (if (contains (make-it-List fast-counters) index) (serve (cdr requests) (update (ttandet+ minutes) fast-counters index) slow-counters)
            (serve (cdr requests) fast-counters (update (ttandet+ minutes) slow-counters index)))
        ]

       [(list 'remove-first)
        (if
          (< (cdr (min-et fast-counters)) (cdr (min-et slow-counters )))
