...
; Veți întoarce coada actualizată.
(define (dequeue q)
  (if (null? (queue-left q)) (struct-copy queue q [left (cdr (reverse (queue-right q)))] [right null] [size-l (- (queue-size-r q) 1)] [size-r 0])
...
(define (top q)
  ...
  (car (append (queue-left q) (reverse (queue-right q))))))
