...
(define (queue-empty? q)
  (if (and (stream-empty? (queue-left q)) (null? (queue-right q))) #t #f))
...
(define (enqueue x q)
  (if (= (queue-size-l q) (queue-size-r q))
      (struct-copy queue q [left (rotate (queue-left q) (append (list x) (queue-right q)) empty-stream)]  [right '()]
                  [size-l (stream-length (rotate (queue-left q) (append (list x) (queue-right q)) empty-stream))]  [size-r 0])
...
(define (dequeue q)
  (if (= (queue-size-l q) (queue-size-r q))
      (struct-copy queue q [left (rotate (stream-rest (queue-left q)) (queue-right q) empty-stream)]  [right '()]
                  [size-l (stream-length (rotate (stream-rest (queue-left q)) (queue-right q) empty-stream))]  [size-r 0])
   ; (if (stream-empty? (queue-left q)) (struct-copy queue q [left (rotate (queue-left q) (queue-right q) empty-stream)] [right null] [size-l (- (queue-size-r q) 1)] [size-r 0])
